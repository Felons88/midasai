...
export async function GET() {
  ...
  try {
    ...
    console.error("nexus/files GET", e)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!name || !path || !type) return NextResponse.json({ error: "name, path, type required" }, { status: 400 })
    const svc = createNexusService(supabase, user.id)
    const directory = await svc.createDirectory({ name, path, type, metadata })
    return NextResponse.json({ file: directory }, { status: 201 })
  } catch (e) {
    console.error("nexus/files POST", e)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
