...
import { Edit, Eye, Plus, DollarSign, MessageSquare, HelpCircle, Terminal } from "lucide-react"
import { ListingRowActions } from "@/components/creator/ListingRowActions"
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" className="transition-smooth" asChild>
                      <Link href={`/creator/listings/${listing.id}/faq`}>
                        <HelpCircle className="h-4 w-4 mr-2" />
                        FAQ
                      </Link>
                    </Button>
                    <Button variant="outline" size="sm" className="transition-smooth" asChild>
                      <Link href={`/creator/listings/${listing.id}/install`}>
                        <Terminal className="h-4 w-4 mr-2" />
                        Install
                      </Link>
                    </Button>
                    <Button variant="outline" size="sm" className="transition-smooth" asChild>
                      <Link href={`/creator/listings/${listing.id}/tags`}>Tags</Link>
                    </Button>
                    <Button variant="outline" size="sm" className="transition-smooth" asChild>
                      <Link href={`/creator/listings/${listing.id}/docs`}>Docs</Link>
                    </Button>
                    <Button variant="outline" size="sm" className="transition-smooth" asChild>
                      <Link href={`/creator/listings/${listing.id}/media`}>Gallery</Link>
                    </Button>
                    <ListingRowActions
