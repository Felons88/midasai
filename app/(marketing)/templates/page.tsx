...
async function getTemplatesListings() {
  try {
    ...
    if (error) {
      console.error('Error fetching templates listings:', error)
      return []
    }
    
    return listings || []
  } catch (error) {
    console.error('Error in getTemplatesListings:', error)
    return []
  }
}

export default async function TemplatesPage() {
  const listings = await getTemplatesListings()
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-12 relative">
        <div className="mb-12 animate-fade-in-up">
          <h1 className="text-5xl md:text-6xl font-bold mb-4 text-text-primary">Templates</h1>
        ...
        {listings.length === 0 && (
          <div className="text-center py-24 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <p className="text-xl text-text-secondary">No templates found.</p>
          </div>
        )}
      </div>
    </div>
