...
async function getPluginsListings() {
  try {
    ...
    if (error) {
      console.error('Error fetching plugins listings:', error)
      return []
    }
    
    return listings || []
  } catch (error) {
    console.error('Error in getPluginsListings:', error)
    return []
  }
}

export default async function PluginsPage() {
  const listings = await getPluginsListings()
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-12 relative">
        <div className="mb-12 animate-fade-in-up">
          <h1 className="text-5xl md:text-6xl font-bold mb-4 text-text-primary">Cursor Rules</h1>
          <p className="text-xl text-text-secondary">
            Enhance your Cursor IDE with custom rules and configurations
          </p>
        </div>
        ...
        {listings.length === 0 && (
          <div className="text-center py-24 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <p className="text-xl text-text-secondary">No plugins found.</p>
          </div>
        )}
      </div>
    </div>
