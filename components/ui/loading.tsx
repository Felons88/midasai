...
export function HorizontalScrollSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x md:grid md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 md:gap-6 md:overflow-visible md:snap-none">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex-shrink-0 w-72 snap-start md:w-auto">
          <LoadingCard />
        </div>
      ))}
    </div>
