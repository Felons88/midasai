...
import { CheckCircle, XCircle, Clock, Loader2, Ban, ChevronDown, ChevronRight, Activity, RotateCcw } from "lucide-react"
...
interface ExecutionHistoryProps {
  ...
  loading?: boolean
  onReplay?: (workflowId: string, inputData: Record<string, unknown>) => void
}
...
export function ExecutionHistory({ executions, loading, onReplay }: ExecutionHistoryProps) {
  const [expanded, setExpanded] = useState<string | null>(null)
  const [copiedId, setCopiedId] = useState<string | null>(null)

  const handleCopyOutput = (execution: WorkflowExecution) => {
    const text = JSON.stringify(execution.output_data ?? {}, null, 2)
        ...
        return (
          ...
          >
            ...
            >
              ...
              {onReplay && (
                <button
                  onClick={e => { e.stopPropagation(); onReplay(execution.workflow_id, (execution.input_data as Record<string, unknown>) ?? {}) }}
                  title="Re-run with same inputs"
                  className="h-6 w-6 flex items-center justify-center rounded-md text-white/20 hover:text-violet-400 hover:bg-violet-500/10 transition-colors flex-shrink-0"
                >
                  ...
                  <div className="mt-3">
                    ...
                    <div className="space-y-1.5">
                      {nodeResults.map((nr, i) => {
                        const nrCfg = STATUS_CONFIG[nr.status as ExecutionStatus] ?? STATUS_CONFIG.completed
                        const NrIcon = nrCfg.icon
                        const hasOutput = nr.output && Object.keys(nr.output).length > 0
                        return (
                          <details key={i} className="rounded-lg bg-white/[0.02] overflow-hidden">
                            <summary className="flex items-center gap-2.5 px-3 py-2 cursor-pointer list-none">
                              ...
                              <span className="text-xs text-white/60 flex-1 truncate font-mono">{nr.node_id}</span>
                              {nr.error && <span className="text-[9px] text-red-400 truncate max-w-24">{nr.error}</span>}
                              <span className="text-xs text-white/30">{formatDuration(nr.duration_ms)}</span>
                              {hasOutput && <ChevronRight className="h-2.5 w-2.5 text-white/20" />}
                            </summary>
                            {hasOutput && (
                              <div className="px-3 pb-2 pt-1 border-t border-white/[0.04]">
                                <pre className="text-[8px] text-emerald-300/70 font-mono whitespace-pre-wrap break-all max-h-24 overflow-y-auto">
