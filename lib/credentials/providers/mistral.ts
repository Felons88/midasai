...
export const mistralProvider: CredentialProvider = {
  config: {
    id: 'mistral',
    name: 'Mistral',
    icon: '🌊',
  ...
  validate(fields) {
    ...
    } else if (!/^[a-zA-Z0-9]{32,}$/.test(fields.apiKey)) {
      errors.apiKey = 'Invalid Mistral API Key format'
    }
  ...
  async test(fields) {
    ...
    try {
      const result = await testHttpRequest('https://api.mistral.ai/v1/models', {
        headers: {
      ...
      if (result.success) {
        return {
          success: true,
          status: 'connected',
          message: 'Successfully connected to Mistral',
      ...
      return {
        ...
        message: result.error || 'Failed to connect to Mistral',
        timestamp: new Date().toISOString()
      }
    } catch (error) {
      return {
