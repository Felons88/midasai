...
export const affinityProvider: CredentialProvider = {
  config: {
    id: 'affinity',
    name: 'Affinity',
    icon: '🔗',
    category: 'communication',
    documentationUrl: 'https://developers.affinity.co/',
  ...
  async test(fields) {
    ...
    try {
      const result = await testHttpRequest('https://api.affinity.co/persons?limit=1', {
        headers: {
      ...
      if (result.success) {
        return {
          success: true,
          status: 'connected',
          message: 'Successfully connected to Affinity',
      ...
      return {
        ...
        message: result.error || 'Failed to connect to Affinity',
        timestamp: new Date().toISOString()
      }
    } catch (error) {
      return {
