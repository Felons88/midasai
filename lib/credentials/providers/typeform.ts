...
export const typeformProvider: CredentialProvider = {
  config: {
    id: 'typeform',
    name: 'Typeform',
    icon: '📋',
    category: 'communication',
    documentationUrl: 'https://developer.typeform.com/getting-started/',
  ...
  async test(fields) {
    ...
    try {
      const result = await testHttpRequest('https://api.typeform.com/me', {
        headers: {
      ...
      if (result.success) {
        return {
          success: true,
          status: 'connected',
          message: 'Successfully connected to Typeform',
          metadata: {
      ...
      return {
        ...
        message: result.error || 'Failed to connect to Typeform',
        timestamp: new Date().toISOString()
      }
    } catch (error) {
      return {
