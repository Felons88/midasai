...
### Cycle 12 — Schema-enabled listing features — ◑ PARTIAL (rest blocked on MCP re-auth)
- **Done (no schema needed):** **Verified Purchase / Verified Download** review badges — computed from real `transactions` (COMPLETED) and `downloads` records and rendered on the listing reviews. Verified in-browser (the existing review now shows a green "Verified Download" badge after a real download record was added). DML works via PostgREST + service-role key even with the MCP down.
- **Still BLOCKED (need DDL via Supabase MCP, currently `needsAuth`):** creator **review responses** (new column), creator **FAQ** (new table), and **multi-platform install-command definitions** (new column/table). No DB password is available and the service-role key can't run DDL via PostgREST, so these can't proceed until the Supabase MCP is re-authenticated in Cursor.
